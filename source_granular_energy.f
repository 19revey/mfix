...
! Source terms to be kept on lhs
      DOUBLE PRECISION :: S1_lhs, S3_lhs, S4_lhs, S6_lhs, kesi
!-----------------------------------------------
...
!      IF (EP_S(IJK,M)>=0.4D0) THEN
         kesi=PI**0.5*1.8D-3*12.D0/C_E*EP_s(IJK,M)*G_0CS(EP_s(IJK,M))*&
         (THETA_M(IJK,M)/PI/0.7238D0)**0.5
!      ELSE
!         kesi=0
!      ENDIF
!         kesi=0

        kesi=2.D0*(1-C_E)/15.D0*(G_0CS(EP_s(IJK,M))-G_0CS(0.49D0))+1.D0
        IF (kesi<1) kesi=1
       
        S4_lhs = (48.d0/DSQRT(PI))*ETA*(ONE-ETA)*ROP_S(IJK,M)*&
         SUM_EpsGo*DSQRT(THETA_M(IJK,M))/D_P(IJK,M) *&
         (ONE+((0.D-2)**2/THETA_M(IJK,M)))/kesi
!           2.71828D0**(-(0.D-2)**2/THETA_M(IJK,M))*&
!           2.71828D0**(-1.268D0*kesi)
...
! local var. for GTSH theory
      DOUBLE PRECISION :: chi, Sgama_lhs, Spsi_rhs  ,kesi
!-----------------------------------------------
...
! linearized (3/2)*rop_s*T*zeta0
       kesi=PI**0.5*(1.89D-3*12.D0/D_P(IJK,M)*EP_S(IJK,M)*EP_SM*CHI*&
        (THETA_M(IJK,M))/PI/0.7238D0)**0.5
