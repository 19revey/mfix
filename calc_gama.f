...
! Calculate gas-solids heat transfer coefficient (Gunn 1978)
                 !IF (.FALSE.) THEN
		 IF (EP_G(IJK)>=0.5D0 .AND. RE<=100 ) THEN 
                     GAMA_GS(IJK,M) = ((-0.46D0+1.77D0*EP_G(IJK)+0.69D0*EP_G(IJK)**2.D0)/(EP_G(IJK)**3.D0)+&
                      (1.37D0-2.4D0*EP_G(IJK)+1.2D0*EP_G(IJK)**2.D0)*RE**0.7D0*PR1O3)*(&
